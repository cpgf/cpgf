...
#define NOT_FROM(to, value) GCHECK(! canFromVariant<to>(value)); \
	GBEGIN_EXCEPTION fromVariant<to>(value); GEND_EXCEPTION(const cpgf::GException &)
#define NOT_FROM_CAST(to, from, value) GCHECK(! canFromVariant<to>(static_cast<from>(value))); \
	GBEGIN_EXCEPTION fromVariant<to>(static_cast<from>(value)); GEND_EXCEPTION(const cpgf::GException &)
...
GTEST(TestVariant_CastFromFloat)
{
	...
	GCHECK(value.getType() == (GVariantType)((uint16_t)GVariantType::byLvalueReference | (uint16_t)GVariantType::vtFloat));
