...
template <typename D, typename Policy>
void buildMetaFile_B2PolygonContact(const cpgf::GMetaDataConfigFlags & config, D _d, const cpgf::GMetaDataNameReplacer * _r, const Policy & _p)
{
    (void)config; (void)_d; (void)_r; (void)_d; (void)_p;
    using namespace cpgf;
    
    buildMetaClass_B2PolygonContact(config, _d, _r, _p);
}
