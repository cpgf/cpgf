...
#ifndef __META_B2CONTACTSOLVER_H
#define __META_B2CONTACTSOLVER_H
...
template <typename D, typename Policy>
void buildMetaFile_B2ContactSolver(const cpgf::GMetaDataConfigFlags & config, D _d, const cpgf::GMetaDataNameReplacer * _r, const Policy & _p)
{
    (void)config; (void)_d; (void)_r; (void)_d; (void)_p;
    using namespace cpgf;
    
    buildMetaClass_B2ContactConstraint(config, _d, _r, _p);
    buildMetaClass_B2ContactConstraintPoint(config, _d, _r, _p);
    buildMetaClass_B2ContactSolver(config, _d, _r, _p);
}
