...
template <typename D, typename Policy>
void buildMetaFile_B2Shape(const cpgf::GMetaDataConfigFlags & config, D _d, const cpgf::GMetaDataNameReplacer * _r, const Policy & _p)
{
    (void)config; (void)_d; (void)_r; (void)_d; (void)_p;
    using namespace cpgf;
    
    buildMetaClass_B2MassData(config, _d, _r, _p);
    buildMetaClass_B2Shape(config, _d, _r, _p);
}
