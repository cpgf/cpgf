...
#ifndef __META_B2PULLEYJOINT_H
#define __META_B2PULLEYJOINT_H
...
template <typename D, typename Policy>
void buildMetaFile_B2PulleyJoint(const cpgf::GMetaDataConfigFlags & config, D _d, const cpgf::GMetaDataNameReplacer * _r, const Policy & _p)
{
    (void)config; (void)_d; (void)_r; (void)_d; (void)_p;
    using namespace cpgf;
    
    buildMetaClass_Global_b2pulleyjoint(config, _d, _r, _p);
    buildMetaClass_B2PulleyJoint(config, _d, _r, _p);
    buildMetaClass_B2PulleyJointDef(config, _d, _r, _p);
}
