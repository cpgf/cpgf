...
void buildMetaClass_CMatrix4(const cpgf::GMetaDataConfigFlags & config, D _d)
{
    ...
    _d.CPGF_MD_TEMPLATE _constructor<void * (CMatrix4<T >::eConstructor)>()
        ._default(copyVariantFromCopyable(CMatrix4<T >::EM4CONST_IDENTITY))
    ;
    _d.CPGF_MD_TEMPLATE _constructor<void * (const CMatrix4< T > &, CMatrix4<T >::eConstructor)>(cpgf::MakePolicy<cpgf::GMetaRuleCopyConstReference<0> >())
